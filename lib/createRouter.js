...
var createRouter = exports.createRouter = function createRouter(module, options) {
    ...
    if(options.bindroot !== false) {
        router.get('/', function() {
          ...
          this.res.end(ws.view.renderRouter.call(this.res, 'html', '', router, template.toString()));
        });
    }
    ...
    router.get('/docs.json', function() {
      this.res.writeHead(200, {'Content-Type': 'text/html'});
      this.res.end(ws.view.renderRouter.call(this.res, 'html', '', router, JSON.stringify(router)));
    });
   ...
   //var endpoint = createEndpoint(router.routes);
