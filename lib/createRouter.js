var ws = require('./webservice'), fs = require('fs'), journey = require('journey'), resourcer = require('resourceful'), validator = require('revalidator');
...
function _extendRouter(map, module, options) {
    ...
    for(var method in module) {
        ...
        var regex = new RegExp('\/(' + method + ')\/([^\/]*?)'), regexfix = new RegExp('\/(' + method + ')'), journeyHandler = _createJourneyHandler(module, method, options);
...
function _createJourneyHandler(module, method, options) {
    ...
    var handler = function(res, resource, id, params) {
        ...
        // 0 - response mock
        // 1 - resource e.g. echo
        // 2 - id (first capture group)
        // 3 - paramaters Object {msg: 'hello'}
        // 4 - callback?

        // If there are more capture groups, they will be inserted before the params
        var params = arguments[arguments.length - 1]; // params will always be the last argument
        var method_options = params || {}, self = this;

        var captures = Array.prototype.slice.call(arguments, 1, arguments.length - 1);
