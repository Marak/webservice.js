        ...
        res.send(200, {'Content-Type': 'text/html'}, { version: journey.version.join('.') });
           ...
           for(var method in module){
               ...
               var handler = function (res, resource, id, params) {
                 ...
                 args.push(options);
                 args.push(function(err, result, response){
                   ...
                   // TODO: pull the content-type from the request and set the response to that
                   if(err){
                     // TODO: create error handler view
                     res.send(response.statusCode, {'Content-Type': 'application/json'}, JSON.stringify(err));
                   }
                   else{
                     result = result || '';
                     res.send(response.statusCode, {'Content-Type': 'application/json'}, JSON.stringify(result.toString()));
                   }
                 });
